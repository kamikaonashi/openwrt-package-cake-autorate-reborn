 ...
 *     [nlattr TCA_CAKE_BASE_RATE64  12 B]  u64 rate_Bps  (bytes/sec)
 *
                          ...
                          uint32_t     rate_kbps)
{
    ...
    /*
     * Convert kbps → bytes/sec.
     * TCA_CAKE_BASE_RATE64 stores the rate in BYTES per second, matching
     * how iproute2's tc encodes it (it calls get_rate64() which returns
     * bits/sec, then divides by 8 before writing the netlink attribute).
     *   kbps * 1000 / 8  ==  kbps * 125
